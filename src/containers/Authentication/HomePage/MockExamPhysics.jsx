...
const MockExamPhysics = () => {
  ...
  const handleStart = async () => {
      ...
      if (!recordsError) {
        ...
        if (questionIds.length === 0 || formData.familiarity === 'All') {
          ...
          const { data: moduleQuestions, error: moduleError } = await supabase
            .from('questions')
            .select('*')
            .eq('sub_category', formData.module);
    ...
    // No questions? Show alert and stop here
    if (limitedQuestions.length === 0) {
      alert('No questions found based on your selected criteria. Please try different options.');
      return;
    }
